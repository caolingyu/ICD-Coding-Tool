...
def save_samples(data, output, target_data, s, filter_size, tp_file, fp_file, freqs, dicts=None):
    """
        ...
        INPUTS:
            ...
            filter_size: size of the convolution filter, and size of phrase to extract from source text
            tp_file: opened file to write true positive results
            fp_file: opened file to write false positive results
            freqs: dict of (code: frequency) to save along with attention results
    ...
    if dicts is not None:
        if s is not None and len(pred_codes) > 0:
            important_spans(data, output, tgt_codes, pred_codes, s, dicts, filter_size, true_str, pred_str, tp_file, freqs, fps=False)
            important_spans(data, output, tgt_codes, pred_codes, s, dicts, filter_size, true_str, pred_str, fp_file, freqs, fps=True)

def important_spans(data, output, tgt_codes, pred_codes, s, dicts, filter_size, true_str, pred_str, spans_file, freqs, 
                    fps=False):
    """
        looks only at the first instance in the batch
    """
    ind2w, ind2c, desc_dict = dicts[0], dicts[2], dicts[4]
    for p_code in pred_codes:
        #aww yiss, xor... if false-pos, save if it's a wrong prediction, otherwise true-pos, so save if it's a true prediction
        if output[0][p_code] > .5 and (fps ^ (p_code in tgt_codes)):
            ...
            freq_str = "frequency of code %s: %f" % (str(p_code), freqs[code])
...
def make_windows(starts, filter_size, attn):
    ...
    while i < len(starts):
        ...
        i += 1
    #want to return windows sorted by decreasing importance
