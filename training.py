...
def init(args):
    ...
    dicts = datasets.load_lookups(args.data_path, args.vocab, desc_embed=desc_embed)
...
def train_epochs(args, model, optimizer, params, freq_params, dicts):
    ...
    for epoch in range(args.n_epochs):
        ...
        metrics_all, unseen_code_inds = one_epoch(model, optimizer, args.Y, epoch, args.n_epochs, args.batch_size, args.data_path,
                                                  freq_params, test_only, dicts, model_dir, unseen_code_inds,
...
def one_epoch(model, optimizer, Y, epoch, n_epochs, batch_size, data_path, freq_params, testing, dicts, model_dir, unseen_code_inds, 
              samples, gpu, debug, quiet):
    """
        Basically a wrapper to do a training epoch and test on dev
    """
    if not testing:
        losses, unseen_code_inds = train(model, optimizer, Y, epoch, batch_size, data_path, gpu, freq_params, dicts, debug, quiet)
        ...
        if model.lmbda > 0:
            ...
            num_labels = tools.get_num_labels(Y)
    ...
    #test on dev
    metrics = test(model, Y, epoch, batch_size, data_path, fold, gpu, unseen_code_inds, dicts, samples, freq_params, model_dir,
                   testing, debug)
    if testing or epoch == n_epochs - 1:
        print("evaluating on test")
        metrics_te = test(model, Y, epoch, batch_size, data_path, "test", gpu, unseen_code_inds, dicts, samples, freq_params,
...
def train(model, optimizer, Y, epoch, batch_size, data_path, gpu, freq_params, dicts, debug, quiet):
    """
        Training loop.
    ...
    model.train()
    gen = datasets.data_generator(data_path, dicts, batch_size, num_labels, desc_embed=desc_embed)
...
def unseen_code_vecs(model, code_inds, dicts):
    """
    ...
    desc_embeddings = model.embed_descriptions([vecs], len(dicts[0]))[0]
    #replace relevant final_layer weights with desc embeddings 
    model.final.data[code_inds, :] = desc_embeddings.data
    model.final_bias.data[code_inds] = 0

def test(model, Y, epoch, batch_size, data_path, fold, gpu, code_inds, dicts, samples, freq_params, model_dir, testing, debug):
    """
        Testing loop.
        Returns metrics
    """
    ...
    model.eval()
    gen = datasets.data_generator(filename, dicts, batch_size, num_labels, desc_embed=desc_embed)
    for batch_idx, tup in tqdm(enumerate(gen)):
        if debug and batch_idx > 50:
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("model", type=str, choices=["cnn_vanilla", "rnn", "conv_attn", "multi_conv_attn", "rnn_attn", "saved"], help="model")
